	...
	return this.each( function() {
		...
		addXmlData = function (xml,t, rcnt, more, adjust) {
			...
			while (j<gl) {
				...
				cn1 = (altr+j)%2 == 1 ? cn : '';
				rowData.push("<tr" + hiderow + " id=\"" + rid + "\" tabindex=\"-1\" role=\"row\" onclick=\"ondblClickRow(" + rid + ")\" class =\"ui-widget-content jqgrow ui-row-" + ts.p.direction + "" + cn1 + "\">");
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			while (i<len) {
				...
				altr = rcnt === 1 ? 0 : rcnt;
				cn1 = (altr+i)%2 == 1 ? cn : '';
				rowData.push("<tr" + hiderow + " id=\"" + idr + "\" tabindex=\"-1\" role=\"row\" onclick=\"ondblClickRow(" + idr + ")\" class= \"ui-widget-content jqgrow ui-row-" + ts.p.direction + "" + cn1 + "\">");
				if(ts.p.rownumbers===true) {
...
$.jgrid.extend({
			...
			if(!$t.p.multiselect) {
				if(pt.className !== "ui-subgrid") {
					if( $t.p.selrow != pt.id) {
						...
						$(pt).addClass("ui-state-highlight").attr({ "aria-selected": true, "tabindex": "0", "onclick": "ondblClickRow("+pt.id+")" }); //.focus();
						stat = true;
					} else {
						stat = false;
					}
	...
	addRowData : function(rowid,rdata,pos,src) {
		...
		row = "<tr id=\"" + rowid + "\" role=\"row\" tabindex=\"-1\"  onclick=\"ondblClickRow(" + rowid + ")\" class=\"ui-widget-content jqgrow ui-row-" + t.p.direction + " " + cna + "\">" + row + "</tr>";
					if(t.rows.length === 0){
	...
	groupingRender : function (grdata, colspans ) {
		return this.each(function(){
			...
			$.each(grp.sortitems[0],function(i,n){
				...
				str += "<tr id=\""+hid+"\" role=\"row\" class= \"ui-widget-content jqgroup ui-row-"+$t.p.direction+"\" onclick = setErrorInfo("+hid+")><td colspan=\""+colspans+"\">"+icon+$.jgrid.format(grp.groupText[0], gv, grdata[n].length)+"</td></tr>";
